...
export default function devOrchestratorExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"After dev_screen_run, do not repeatedly poll while jobs are active. Continue safe work or wait for the dev-run-watch completion follow-up; inspect exit-code metadata only after completion, when explicitly asked for status, or when a notification appears missed.",
			"Before claiming validation passed, inspect exit-code metadata and read only log tails unless full logs are necessary.",
		],
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"After dev_experiment_run, do not repeatedly poll while jobs are active. Continue safe work or wait for the dev-run-watch completion follow-up before reading SUMMARY.md.",
			"Use /dev-experiment-status or inspect SUMMARY.md early only when explicitly asked for status, when no safe work remains, or when a notification appears missed.",
		],
		parameters: Type.Object({
